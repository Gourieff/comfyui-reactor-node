...
- ReActorFaceBoost Node - an attempt to improve the quality of swapped faces. The idea is to scale and restore the swapped face (according to the `face_size` parameter of the restoration model) BEFORE pasting it to the target image (via inswapper algorithms), more information is [here (PR#321)](https://github.com/Gourieff/comfyui-reactor-node/pull/321)
